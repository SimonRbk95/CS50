...
def index():
    ...
    stocks_owned = db.execute("SELECT symbol, SUM(SELECT quantity FROM portfolio WHERE quantity > 0) AS 'quantity' FROM portfolio WHERE user_id = (?) GROUP BY symbol", session["user_id"])
